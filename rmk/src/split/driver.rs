    ...
    PeripheralManager<ROW, COL, ROW_OFFSET, COL_OFFSET, T>
{
    ...
    pub(crate) async fn run(mut self) {
        ...
        let mut user_state_sub = crate::event::UserStateEvent::subscriber();
        #[cfg(feature = "_ble")]
        ...
        loop {
            ...
            let next_event_to_peri = async {
                crate::select_biased_with_feature! {
                    ...
                    e = user_state_sub.next_event().fuse() => SplitMessage::UserState { id: e.id, value: e.value },
    ...
    async fn process_peripheral_message(&self, split_message: SplitMessage) {
        ...
        match split_message {
            ...
            SplitMessage::BatteryStatus(state) => {
                ...
                let event = PeripheralBatteryEvent { id: self.id, state };
                crate::ble::battery_service::cache_peripheral_battery_status(event);
                publish_event(event)
            }
